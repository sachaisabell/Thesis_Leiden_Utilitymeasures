...
plot_func = function(data, title, type, xlimits, ylimits, bin = NULL, fill_limits = NULL){
  ...
  if(type == "heatmap") {
    p = p + ggdensity::geom_hdr(data = data, aes(x, y), probs = seq(0.00001,0.99,0.1))
  }
  
  if(type == "grid") {
    # For grid, data is expected to be pre-binned with x,y centers and counts (count column)
...
##### and now uncomment to save the maps as images
...
#### application etc.
####
consistent_maps$grid[1:5]
normal_maps$point[1:5]

consistent_maps$raster_data$original
ras <- raster(ncol = 35, nrow = 35);extent(ras) = unlist(consistent_maps$grid$grid$coordinates$limits)
values(ras) = consistent_maps$raster_data$original$count
plot(ras, asp = 1)

ras.mat = raster::as.matrix(ras)

#### testing the hotspots
hs.grid = grid_hotspot(ras.mat,0.9)
plot(hs.grid)
