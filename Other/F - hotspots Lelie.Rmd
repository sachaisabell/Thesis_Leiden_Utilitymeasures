...
### Converting to (bounded) hotspots
```{r}
...
And matching the hotspots to each other (from safe -> original)
```{r}
s2o = st_nearest_feature(hotspot_safe$geometry, hotspot_og$geometry)
...
```{r}
# turning hotspots from polygons into coordinate matrices
...
nrow(coordinates_matrix(hotspot_og))
...
box_counting_dim_func = function(safe, original, eps = 10){
  boxcount_safe = sapply(2:eps, function (x) box_counting(x, safe$geometry))
  boxcount_og = sapply(2:eps, function (x) box_counting(x, original$geometry))
...
Application:
```{r}
output_measure(box_counting_dim_func, hotspot_safe, hotspot_og, "", eps = 10)
...
### Angle measure (for grids only) NOTE: 'matches' is redefined here
Making grid hotspots
```{r}
