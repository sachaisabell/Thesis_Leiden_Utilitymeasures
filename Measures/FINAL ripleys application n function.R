...
all_ripleys_func_sep = function(original, safe, maplim, title = NA){
  df = original
  dfsafe = safe
...
all_ripleys_func = function(original, safe, maplim, title = "NA"){
  df = original
  ...
  jpeg(paste0(title,"KLetc.jpeg"), width = 2000, height = 1000)
  par(mfrow = c(1,3))
  ...
  jpeg(paste0(title,"kpi.jpeg"), width = 2000, height = 1000)
  par(mfrow = c(1,2))
  ...
  ### plot D function
  jpeg(paste0(title,"Dncross.jpeg"), width = 2000, height = 1000)
...
## normal data
all_ripleys_func(normal_maps$point_data$original, normal_maps$point_data[2:5], normal_maps$point$original, "norm")

## clustered data
all_ripleys_func(clustered_maps$point_data$original,clustered_maps$point_data[2:5], clustered_maps$point$original,"clus")

## random data
all_ripleys_func(random_maps$point_data$original,random_maps$point_data[2:5], random_maps$point$original,"rand")

## small clusters data
all_ripleys_func(smallclusters_maps$point_data$original,smallclusters_maps$point_data[2:5], smallclusters_maps$point$original,"small")

## consistent data
all_ripleys_func(consistent_maps$point_data$original,consistent_maps$point_data[2:5], consistent_maps$point$original,"cons")
