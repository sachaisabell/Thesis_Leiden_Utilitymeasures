...
boxcount_plot = function(original, safe,title, eps = 5){
  ...
  lines(safe, c(log(2*2^(1:eps-1))), type = "l", col = "red")
  legend("topleft",legend = c("Original","Safe"), col = c("blue","red"), pch = 15)
}

datanames = c("normal") #c("smallclusters", "consistent","random","normal","clustered")

## takes quite a long times for epsilon
for (i in 1:length(datanames)) {
  epsilon = 5
  current_hotspot = get(paste0(datanames[i], "_hotspots"))
  out = lapply(current_hotspot[2:5], function(x) output_measure(box_counting_dim_func, x, current_hotspot[[1]], "", eps = epsilon))
  ...
  jpeg(paste0(datanames[i],"_Boxcount5.jpg"), width = 600, height = 350)
  par(mfrow = c(1,4))
  boxcount_plot(random.o, random.s, title = paste(datanames[i],"-","Random"),epsilon)
  boxcount_plot(grid.o, grid.s, title = paste(datanames[i],"-","Grid"),epsilon)
  boxcount_plot(vonoroi.o, vonoroi.s, title = paste(datanames[i],"-","Vonoroi"),epsilon)
  boxcount_plot(weighted.o, weighted.s, title = paste(datanames[i],"-","Weighted"),epsilon)
  dev.off()
}
